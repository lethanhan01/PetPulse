export function Pagination({ page, totalPages, setPage }: { page: number; totalPages: number; setPage: (page: number) => void }) {
  ...
  return <nav className="mt-5 flex items-center justify-center gap-2" aria-label="Phân trang">
    <button type="button" disabled={page === 1} onClick={() => setPage(page - 1)} className="rounded-lg border border-border px-3 py-2 text-sm disabled:cursor-not-allowed disabled:opacity-50 hover:bg-secondary">Trước</button>
    ...
    <button type="button" disabled={page === totalPages} onClick={() => setPage(page + 1)} className="rounded-lg border border-border px-3 py-2 text-sm disabled:cursor-not-allowed disabled:opacity-50 hover:bg-secondary">Sau</button>
  </nav>;
}
